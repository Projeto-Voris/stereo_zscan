...
class InverseTriangulation:
    ...
    def fringe_masks(self, uv_l, uv_r, std_l, std_r, phi_id, min_thresh=0, max_thresh=0.12):
        """
        ...
        # Encontra o índice do menor valor por linha (por ponto Nc)
        phi_min_id = cp.argmin(diff_phi, axis=1)  # shape: (Nc,)
            std_l: STDn1
        ...
        mask = valid_uv & phi_mask & valid_mod

        return mask

    def fringe_process(self, points_3d: ndarray, mod_thresh) -> ndarray:
        """
        ...
        batch_size = int((self.max_gpu_usage * 1024 ** 3) // bytes_per_batch // 2)  # margem de segurança
        ...
        for i in range(0, Nc, batch_size):
            ...
            # Máscara de validade
            mask = self.fringe_masks(uv_l = uv_left, uv_r = uv_right, std_l = std_left, std_r = std_right, phi_id = phi_min_id)
