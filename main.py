...
def main():
    ...
    for n_img in n_imgs_v:
        ...
        points_3d = Zscan.points3d(x_lim=(-600, 600), y_lim=(-600, 600), z_lim=(-800, 800), xy_step=15, z_step=2,
                                   visualize=False)
        ...
        points_3d_2 = Zscan.points3d(x_lim=xlim, y_lim=ylim, z_lim=zlim, z_step=.5, xy_step=5, visualize=False)
        ...
        split_correl = np.array_split(correl_points, 6)
        output_3d = []
        for i, split in enumerate(split_correl):
            xlim, ylim, zlim = [min(split[:,0]), max(split[:,0])], [min(split[:,1]), max(split[:,1])], [min(split[:,2]), max(split[:,2])]
            points_3d = Zscan.points3d(x_lim=xlim, y_lim=ylim, z_lim=zlim, z_step=.01, xy_step=1, visualize=False)
            print('{} - X: {}x{}, Y: {}x{}, Z:{}x{}'.format(i, xlim[0], xlim[1], ylim[0], ylim[1], zlim[0], zlim[1]))
            print('{} - 3D meshgrid pts: {} mi'.format(i,points_3d.shape[0]/1e6))
            correl_points = Zscan.correlation_process(points_3d=points_3d, visualize=True, save_points=False,
                                                      win_size=7, threshold=0.8)
